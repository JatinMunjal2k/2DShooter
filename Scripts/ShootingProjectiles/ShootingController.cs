...
public class ShootingController : MonoBehaviour
{
    ...
    public GameObject homingMissilePrefab = null;
    public Transform homingMissileHolder = null;
    ...
    public void SpawnHomingMissile()
    {
        if (homingMissilePrefab != null) {
            bool shouldSpawn = Random.Range(0f, 1f) < homingMissileProbability;
            if (!shouldSpawn) {
                return;
            }

            // TODO: randomize the location of missile
            GameObject missile = Instantiate(homingMissilePrefab, transform.position, transform.rotation, null);
            if (homingMissileHolder != null) {
                missile.transform.SetParent(homingMissileHolder);
            }
        }
    }
}
